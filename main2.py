...
div class='full-header'
    div style='display flex; align-items center;'
        img src='httpsimg.icons8.comcolor48dashboard-layout.png' style='margin-right 12px;'
        h2Rig Comparison Dashboardh2
    div
    div style='font-size 0.9rem;'Powered by ProdigyIQdiv
div
, unsafe_allow_html=True)
...
with tabs[0]
    ...
    fig = px.bar(metric_data, x=Well_Name, y=Value, title=fWell Name vs {selected_metric})
...
with tabs[1]
    ...
    if DSRE in subset.columns
        try
            ...
            st.error(fChart rendering error {e})
    else
    ...
    if ratio_cols
        try
            ...
            st.error(fError rendering ratio comparison chart {e})
    else
...
with tabs[2]
    ...
    with k5
        ...
        st.metric(⛏️ Max Depth, f{max_depth,.0f} if pd.notnull(max_depth) else NA)

    with k6
        avg_lgs = filtered[Average_LGS%].mean() if Average_LGS% in filtered.columns else None
        st.metric(🌀 Avg LGS%, f{avg_lgs.2f} if pd.notnull(avg_lgs) else NA)
  ...
  - 🛠️ Converts data into decisions (e.g., adjust mud ratios, optimize bit hydraulics).
    ...
    if ROP in filtered.columns and Temp in filtered.columns
        try
            ...
            st.error(fError rendering ROP vs Temp chart {e})
    else
    ...
    if Base_Oil in filtered.columns and Water in filtered.columns
        try
            ...
            st.error(fError rendering Base Oil vs Water chart {e})
    else
    ...
    try
        ...
        st.error(fCorrelation heatmap error {e})
